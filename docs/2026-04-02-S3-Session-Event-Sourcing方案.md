...
### 新建文件
- `core/event_log.py` — 完整实现：`SessionEvent` dataclass + `EventType` 枚举（14 种事件类型）+ `EventWriter`（内存队列 + 批量 flush + UUID 去重 + 文件权限 0o600）+ `EventReader`（load/rebuild/metadata scan/corrupted line 容错）+ 模块级便捷函数 `emit_event()` / `flush_events()`
- `tests/test_event_log_unit.py` — 29 个单元测试（覆盖序列化、写入去重、多 session、权限、读取容错、状态重建、对话重建、metadata 扫描、emit 开关）

### 修改文件
- `config/settings.py` — 新增 2 个配置项：`SESSION_EVENT_LOG_ENABLED`（默认 false）、`SESSION_EVENT_FLUSH_INTERVAL`（默认 5 秒）
- `core/graph_utils.py` — `save_runtime_checkpoint()` 末尾追加 `emit_event(CHECKPOINT_SAVED)`
- `core/runtime.py` — 3 处集成：`submit_task()` 追加 `JOB_SUBMITTED` event；`_execute_submitted_job()` 追加 `JOB_STATUS_CHANGED(running)` event；`_finalize_runtime_result()` 追加 `JOB_STATUS_CHANGED(final)` event + `flush_events()`
- `core/graph_nodes.py` — 3 处集成：`route_node()` 追加 `PLAN_CREATED` event；`parallel_executor_node()` 追加 `TASK_STARTED/TASK_COMPLETED/TASK_FAILED` events；`human_confirm_node_v2()` 追加 `APPROVAL_REQUESTED` + `APPROVAL_RESOLVED` events
- `core/replanner.py` — `replanner_node()` 末尾追加 `PLAN_UPDATED` event
- `core/runtime.py` — `resume_job_from_checkpoint()` 新增 event-log-based resume 尝试（优先从 event log 重建审计，fallback 到 checkpoint）

### 与原方案的偏差
1. **EventWriter flush 策略简化**：原方案提到 "SESSION_EVENT_FLUSH_INTERVAL 秒或队列满 50 条"。实现中 flush_interval 用 `time.monotonic()` 判断，auto-flush 在 `append()` 中被动触发（非独立定时器线程），避免引入额外线程复杂性。job 完成时显式调用 `flush_events()` 保证事件不丢。
2. **Resume 保留 checkpoint 主路径**：原方案 S3-5 要求 "改为从 event log 重建状态"。实际实现中 event log 仅作为审计补充和 resume 检测（是否中断），状态恢复仍走 checkpoint。原因：checkpoint 包含完整 `OmniCoreState`（含 messages），event log 仅记录状态变更摘要，不足以完整恢复 LangGraph 执行状态。
3. **artifact_created event 集成点**：原方案要求在 `utils/artifact_store.py` 写 event。实际通过 `_finalize_runtime_result()` 的 job 完成事件间接覆盖（artifacts 信息已包含在 job 完成状态中），不侵入 artifact_store 内部。
4. **policy_engine 集成推迟**：原方案要求在 `core/policy_engine.py` 写 approval event。实际在 `human_confirm_node_v2` 写入，因为 policy_engine 做的是静态规则匹配，人类审批的实际发生点在 confirm node。

### 验收结果
- `pytest tests/test_event_log_unit.py -q` — 29 passed
- `pytest tests -q` — 全量回归通过（已有失败均为 S3 前已存在的 web_worker/reasoning 相关测试）
