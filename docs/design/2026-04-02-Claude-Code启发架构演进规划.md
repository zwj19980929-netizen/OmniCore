...
| S2 | 上下文预算制 + 压缩重注入 | **P0** | 2 天 | [S2 方案](2026-04-02-S2-上下文预算制与压缩重注入方案.md) | ✅ 已完成（2026-04-02） |
| S3 | Session Event Sourcing（append-only event log） | **P1** | 3-4 天 | [S3 方案](2026-04-02-S3-Session-Event-Sourcing方案.md) | ✅ 已完成（2026-04-03） |
| S4 | Tool 六阶段执行 Pipeline | **P1** | 2-3 天 | [S4 方案](2026-04-02-S4-Tool执行Pipeline方案.md) | ✅ 已完成（2026-04-03） |
| S5 | 多 Agent 协作（Coordinator + Subagent） | **P2** | 4-5 天 | [S5 方案](2026-04-02-S5-多Agent协作方案.md) | ✅ 已完成（2026-04-03） |
| S6 | Fail-Closed 安全分层 | **P2** | 2 天 | [S6 方案](2026-04-02-S6-Fail-Closed安全分层方案.md) | ✅ 已完成（2026-04-03） |
...
**完成记录**（2026-04-02）：
- 修改 `utils/context_budget.py`（+`ContextBudget` 预算分配器 + `AutoCompactor` 压缩器 + circuit breaker + PTL fallback）
- 新建 `core/context_reinject.py`（压缩后重注入：plan + artifacts + tools + session memory）
- 修改 `config/settings.py`（+3 配置项：`CONTEXT_RESERVE_TOKENS`/`CONTEXT_COMPACT_THRESHOLD`/`COMPACT_MAX_CONSECUTIVE_FAILURES`）
- 修改 `core/graph_nodes.py`（`route_node`/`parallel_executor_node` 入口集成 auto-compact）
- 修改 `core/replanner.py`（`replanner_node` 入口集成 auto-compact）
- 新建 `tests/test_context_reinject_unit.py`（10 个测试）
- 扩展 `tests/test_context_budget_unit.py`（+21 个 S2 测试）
- 全量测试通过
...
**完成记录**（2026-04-03）：
- 新建 `core/event_log.py`（SessionEvent + EventType 14 种 + EventWriter 内存队列批量 flush + EventReader 状态重建/对话重建/metadata 扫描）
- 新建 `tests/test_event_log_unit.py`（29 个测试）
- 修改 `config/settings.py`（+2 配置项：`SESSION_EVENT_LOG_ENABLED`/`SESSION_EVENT_FLUSH_INTERVAL`）
- 修改 `core/graph_utils.py`（`save_runtime_checkpoint` 追加 checkpoint event 写入）
- 修改 `core/runtime.py`（`submit_task` / `_execute_submitted_job` / `_finalize_runtime_result` / `resume_job_from_checkpoint` 集成 event 写入与 event-log resume）
- 修改 `core/graph_nodes.py`（`route_node` plan_created / `parallel_executor_node` task events / `human_confirm_node_v2` approval events）
- 修改 `core/replanner.py`（`replanner_node` plan_updated event）
- 全量测试通过
...
**完成记录**（2026-04-03）：
- 新建 `core/subagent.py`（SubagentSpec + SubagentResult + SubagentRunner，独立 graph 循环、上下文继承、资源限制、depth 控制、并行执行 + fail_fast/best_effort 策略）
- 新建 `core/coordinator.py`（coordinator_node LLM 分解 → 并行 dispatch → 合成、degradation 降级逻辑）
- 修改 `core/router.py`（analysis 输出新增 `needs_coordinator` 字段，LLM 驱动 coordinator 路由判定）
- 修改 `prompts/router_system_static.txt`（JSON 输出格式 + 关键原则新增 coordinator 判定指引）
- 新建 `prompts/coordinator_system.txt`（Coordinator 角色 prompt）
- 新建 `tests/test_subagent_unit.py`（20 个测试）
- 新建 `tests/test_coordinator_unit.py`（26 个测试）
- 修改 `core/message_bus.py`（+7 个 S5 消息类型常量）
- 修改 `config/settings.py`（+6 配置项：COORDINATOR_ENABLED/MAX_SUBAGENT_DEPTH/MAX_PARALLEL_SUBAGENTS/SUBAGENT_MAX_TURNS/SUBAGENT_TIMEOUT/SUBAGENT_FAILURE_STRATEGY）
- 修改 `config/agents.yaml`（+coordinator agent 类型）
- 修改 `core/graph_nodes.py`（+coordinator_node 注册 + route_node 中 coordinator 判定注入）
- 修改 `core/graph_conditions.py`（should_continue_after_route 新增 coordinator 分支）
- 修改 `core/graph.py`（legacy + registry graph 均添加 coordinator 节点和边）
- 全量测试通过
- 注意：权限桥接消息类型已定义，实际审批流打通待 S6 安全分层集成
...
**完成记录**（2026-04-03）：
- 修改 `core/tool_protocol.py`（ToolSpec 默认值 fail-closed：`concurrent_safe=False`、`destructive=True`；新增 `trust_level` 字段）
- 修改 `core/tool_registry.py`（内置工具显式声明 trust_level + destructive；MCP 注册新增 description 截断、名称冲突检查、trust_level 自动标记；register() 新增 debug warning）
- 修改 `core/mcp_client.py`（连接超时保护、认证失败缓存 15min、startup 总超时）
- 修改 `core/policy_engine.py`（MCP trust_level 接入：mcp_remote + destructive 强制人工审批）
- 修改 `core/tool_pipeline.py`（终端命令黑名单、权限阶段接入 trust_level、审计日志 + 敏感参数脱敏）
- 修改 `config/settings.py`（+8 配置项：MCP_DESCRIPTION_MAX_LENGTH / MCP_TRUST_LEVEL / ALLOWED_DOMAINS / AUDIT_LOG_ENABLED / MCP_AUTH_FAILURE_CACHE_SECONDS / MCP_CONNECT_TIMEOUT / MCP_STARTUP_TIMEOUT）
- 新建 `tests/test_security_unit.py`（33 个测试）
- 全量测试通过
...
| `COORDINATOR_ENABLED` | `false` | S5 | 是否启用 Coordinator 模式 |
| `MAX_SUBAGENT_DEPTH` | `1` | S5 | 子 agent 最大嵌套深度 |
| `MAX_PARALLEL_SUBAGENTS` | `3` | S5 | 同时运行的子 agent 数量上限 |
| `SUBAGENT_MAX_TURNS` | `10` | S5 | 单个子 agent 最大执行轮次 |
| `SUBAGENT_TIMEOUT` | `300` | S5 | 单个子 agent 超时时间（秒） |
| `SUBAGENT_FAILURE_STRATEGY` | `best_effort` | S5 | 子 agent 失败策略（fail_fast/best_effort） |
