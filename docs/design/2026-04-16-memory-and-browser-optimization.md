...
> Status: in-progress (A 组 A1~A5 全部上线并收口;B1 数据层 + 决策注入 + 执行层 fallback 注入 + login_replay + record_template 尾部抓取已落地;B2 数据层 + BrowserToolkit 接入(UA/delay/response listener/正反馈)已落地;B3 已上线;B4 感知暴露 + 决策注入 + 自动扫 iframe + tab 回收已落地;B5 失败策略自适应学习已上线;B6 StrategyPicker/PageAssessmentCache/三策略 + login_replay 策略已接入,默认关闭)
> Created: 2026-04-16
...
| B1 | 站点选择器 + 登录流记忆 | ✅ 数据层 + 决策注入 + 执行层 fallback 注入 | off (主开关 `BROWSER_PLAN_MEMORY_ENABLED`) | 19 + 10 |
| B2 | 反爬 domain 画像 | ✅ 数据层 + block 反馈 + BrowserToolkit 接入(UA/delay/response listener/正反馈) | off (主开关 `ANTI_BOT_PROFILE_ENABLED`) | 15 + 10 |
| B3 | 视觉验证缓存 | ✅ 上线 | off (`BROWSER_VISION_CACHE_ENABLED`) | 37 |
| B4 | iframe / 多 tab 支持 | ✅ 感知暴露 + 决策 prompt + auto-scan 兜底 + tab 回收 | 感知 on / auto-scan off | 10 |
| B5 | 失败策略自适应学习 | ✅ 上线 | off (主开关 `BROWSER_STRATEGY_LEARNING_ENABLED`) | 16 |
| B6 | 三模式解耦重构 | ✅ 策略链接入(legacy/unified/batch/login_replay) + PageAssessmentCache per-run 挂载 + 尾部 template 抓取 + site_hints 模板变量化 | off (主开关 `BROWSER_STRATEGY_REFACTOR_ENABLED`) | 24 + 23 + 16 |
...
### B 组下次接手清单(2026-04-16 切分)

**B1 遗留**:
- [x] ~~执行层 fallback 链注入~~(2026-04-17 与 B5 合并落地,见 §B1.7 / §B5.7)
- [x] ~~`agents/browser_login_replay.py`~~(2026-04-18,`try_replay_login(agent, domain, credentials=None, max_fail_count=3)`;placeholder 替换 `{{username}}` / `{{password}}`;每步支持 `expected_checkpoint` 调 `verify_dom_checkpoint` 校验;失败自动 `record_login_flow(success=False)`;成功 `record_login_flow(success=True)`。`tests/test_browser_login_replay_unit.py` 16 条全部通过)
- [x] ~~BrowserAgent.run 尾部自动抓取 `record_template`~~(2026-04-18,`utils/browser_template_recorder.py` 的 `record_template_from_run` 在 B6 `_run_with_strategies` 尾部统一调用;仅针对 `search` / `navigate` / `form` 意图,且只抓取 success==True 的步骤。`tests/test_browser_template_recorder_unit.py` 23 条全部通过)
- [x] ~~`{site_hints}` 从 append-after-format 迁入模板变量~~(2026-04-18,`prompts/browser_act.txt` / `prompts/browser_action_decision.txt` 末尾加 `{site_hints}` 占位符;`browser_decision.py` 两处 `.format()` 改为传 `site_hints=self._build_site_hints_block(...)` kwarg;102 条浏览器单测回归全绿)

**B2 遗留(数据层 + BrowserToolkit 接入均已完成)**:
- [x] ~~`utils/browser_toolkit.py` `new_context` / `goto` 接 throttle hint(UA 覆盖 + delay)~~(2026-04-17,见 §B2.7)
- [x] ~~Playwright response listener 429/503 → `record_block`~~(2026-04-17,见 §B2.7)
- [x] ~~成功 `goto` 路径 `record_request(True)` 正反馈~~(2026-04-17,见 §B2.7)
- [ ] `web_worker._fetch_page_content` 等更上游的成功边界是否需要补 `record_request(True)`:目前 `BrowserToolkit.goto` 层已有正反馈,但 `web_worker` 里的静态抓取路径、search redirect 等非 BrowserToolkit 路径还没覆盖,上线一段时间看画像分布再决定要不要再加一层

**B3~B6(按优先级推荐)**:
- B3 视觉验证缓存:✅ 已上线(2026-04-17),默认关闭,主开关 `BROWSER_VISION_CACHE_ENABLED`
- B5 失败策略自适应学习:✅ 已上线(2026-04-17),与 B1 执行层 fallback 注入合并落地,主开关 `BROWSER_STRATEGY_LEARNING_ENABLED`
- B4 iframe/多 tab:建议与 B2 "new_context 读 throttle 应用"合并落地,共用 BrowserToolkit 改造
- B6 三模式解耦:✅ 已上线(2026-04-18),默认关闭,主开关 `BROWSER_STRATEGY_REFACTOR_ENABLED`。实现采用"薄包装"策略:`_run_with_strategies` 作为新入口,`StrategyPicker.build_chain` 按(LoginReplay → Batch/Unified → Legacy)顺序组装,每个策略返回 `None` 表示"跳过给下一条"。三个旧分支通过 `_run_per_step_loop` / `_run_batch_mode` 复用而非重写,避免 3000 行文件重构风险。尾部统一跑 `record_template_from_run`(B1.2)。
- PageAssessmentCache 接入(2026-04-18):`BrowserAgent.__init__` 挂 per-instance `self._assessment_cache = PageAssessmentCache(max_entries=16)`,`run()` 开头 `clear()` 保证每次 run 从零开始;暴露 `get_or_compute_assessment(page_hash, compute_fn)` 作为策略面 passthrough。当前观察流水线(`_last_observation`)已在单策略内做了观察复用,本次仅提供"跨策略共享"的挂载点,等 LoginReplay → Legacy 这类 hand-off 场景上线后再按需在具体策略内调用,避免过度工程化。
...
#### B1.7 落地情况(2026-04-16)

**已落地(数据层 + 决策注入)**:
- 新增 `utils/site_knowledge_store.py`:
  - SQLite 三表(`site_selectors` / `site_login_flows` / `site_action_templates`),主键 + 索引完备
  - `SiteKnowledgeStore`:线程安全、懒加载、`BROWSER_PLAN_MEMORY_ENABLED=false` 时写读全部 no-op
  - API:`record_selector_success` / `record_selector_failure` / `get_selector_hints(role=, limit=)` / `record_login_flow` / `get_login_flow` / `record_template` / `get_templates`
  - `get_selector_hints` 按 `BROWSER_SELECTOR_MIN_SUCCESS_RATE` 过滤 + `BROWSER_SELECTOR_DECAY_DAYS` 时间衰减,按 (success_rate, hit_count) 降序
  - `record_login_flow(..., success=False)` 只增 `fail_count` 不覆盖 `flow_json`;单个 domain 最多一条登录流(主键 = domain)
  - 进程单例 `get_site_knowledge_store()`(禁用时返回 None)
- `agents/browser_decision.py`:
  - `_build_site_hints_block(url)`:渲染 "Site hints (reference only)" 块,注入到 `_act_with_llm` 与 `_decide_action_with_llm` 两个 prompt 末尾
  - `_record_site_selector_from_action(action)`:`record_action` 末尾自动写回成功 selector(action_type 作为 role bucket)
  - 两个新增方法都按 `BROWSER_PLAN_MEMORY_ENABLED` 守卫,默认零开销
- `config/settings.py` 新增:`BROWSER_SITE_KNOWLEDGE_DB` / `BROWSER_SELECTOR_HINT_TOP_K` / `BROWSER_LOGIN_REPLAY_ENABLED` / `BROWSER_SELECTOR_MIN_SUCCESS_RATE` / `BROWSER_SELECTOR_DECAY_DAYS` / `BROWSER_SITE_HINTS_INJECT`

**追加落地(2026-04-17,与 B5 合并)**:执行层 fallback 链级别注入
- `agents/browser_execution.py`:
  - 新增 helper:`_canonical_strategy_name` / `_current_domain` / `_load_site_hint_selectors` / `_reorder_strategies` / `_record_strategy_outcome` / `_record_site_hint_outcome`
  - `try_click_with_fallbacks`:构建完 strategies 后,按 `get_selector_hints(domain, role="click")` 在最前面插入 `site_hint:<selector>` 策略;随后按 `strategy_stats.ranked_strategies` 重排 + `skip_strategies` 过滤
  - `try_input_with_fallbacks`:同上,site_hint 用 `toolkit.input_text(hint_selector, value)` 包装
  - 每条 strategy 尝试完成后调 `strategy_stats.record(canonical_name, success, latency_ms)`;canonical 化(`role:button:Submit` → `role`)保证 bucket 稳定
  - site_hint 成功时 `record_selector_success` 回写 hint;失败时 `record_selector_failure` 降权
  - **开关**:B1 执行层注入由 `BROWSER_SITE_HINTS_EXEC_INJECT`(默认 true)+ `BROWSER_PLAN_MEMORY_ENABLED`(默认 false)双重守卫
- `config/settings.py` 新增:`BROWSER_SITE_HINTS_EXEC_INJECT`

**尚未落地(下一迭代)**:
- `agents/browser_login_replay.py`(按登录流模板沿着 DOM checkpoint 执行):已留数据层接口(`get_login_flow`),等待决策层+执行层串通后再加实际 replay 路径
- 登录/搜索任务完成后 `record_template(domain, name, sequence)` 自动抓取:需要 BrowserAgent.run 尾部挂钩,待 B6 三模式解耦时做

**测试**:`tests/test_site_knowledge_store_unit.py`(15 条)+ `tests/test_browser_site_hints_unit.py`(6 条)+ `tests/test_browser_execution_fallback_unit.py`(10 条,与 B5 共享)
...
#### B2.7 落地情况(2026-04-16)

**已落地(数据层 + block 反馈)**:
- 新增 `utils/anti_bot_profile.py`:
  - `DomainProfile` / `ThrottleHint` dataclasses
  - `AntiBotProfileStore`:SQLite 单表 `anti_bot_domains`,线程安全、懒加载、feature-flag no-op
  - API:`record_request(domain, success)` / `record_block(domain, kind)` / `set_preferred_ua(domain, ua)` / `get_profile(domain)` / `suggest_throttle(domain)` / `list_profiles()`
  - **退避**:`record_block` 触发指数回退(`current_delay_sec = min(max(prev*2, initial), max)`),同步把 `prefers_headed=1`
  - **冷却**:连续 `ANTI_BOT_SUCCESS_TO_COOLDOWN` 次 `record_request(success=True)` 后 `current_delay_sec` 归零
  - **时间衰减**:`suggest_throttle` 如果 `last_block_at` 比 `ANTI_BOT_BLOCK_DECAY_DAYS` 早则 delay 归零
  - UA 池 `_load_ua_pool()` / `pick_ua(group="desktop_chrome")`:进程级缓存,异常全吞
- 新增 `config/ua_pool.yaml`:按 `desktop_chrome` / `desktop_firefox` / `desktop_safari` / `mobile_chrome` 分组
- `agents/web_worker.py`:
  - `_record_anti_bot_block(url, kind)` 助手方法(feature-flag no-op)
  - 三个检测点调用 `_record_anti_bot_block(url, kind="captcha")`:搜索结果等待 + 候选 URL 重试 + 非搜索 URL 抓取(line ~1915 / ~3480 / ~3663)
- `config/settings.py` 新增:`ANTI_BOT_PROFILE_ENABLED` / `ANTI_BOT_PROFILE_DB` / `ANTI_BOT_INITIAL_DELAY_SEC` / `ANTI_BOT_MAX_DELAY_SEC` / `ANTI_BOT_UA_POOL_FILE` / `ANTI_BOT_BLOCK_DECAY_DAYS` / `ANTI_BOT_SUCCESS_TO_COOLDOWN`

**BrowserToolkit 接入(2026-04-17)**:
- `utils/browser_toolkit.py`:
  - `__init__` 新增 `_ua_override` / `_pending_delay_sec` / `_response_listener_installed` 三个运行时字段
  - `apply_throttle_hint(hint)`:接收 `ThrottleHint`,把 `ua` / `delay_sec` 写进内部字段(叠加多次 hint 时取较大延迟)
  - `create_page()` 用 `self._ua_override or default_ua` 覆盖原硬编码 UA,末尾 `_install_response_listener(page)` 注册 `page.on("response", ...)`;主文档 429 → `record_block(rate_limit)`,503 → `record_block(service_unavailable)`
  - `goto(url)` 开始前 `_apply_pending_delay()` 让之前 hint 里的延迟生效,成功回 `_record_nav_outcome(final_url, True)` 写正反馈
  - `new_tab` / `switch_tab` / `close_tab` 切换 page 后都会重新挂 response listener(避免新 tab 漏监听)
- `agents/browser_agent.py::_initialize_session`:在 `tk.create_page()` 前读 `suggest_throttle(expected_url)`,若 `hint.headed=True` 且当前 headless,翻转 `self.toolkit.headless=False`(Pool key 切到 headed 分支),调 `apply_throttle_hint(hint)`;全流程 gate 在 `ANTI_BOT_PROFILE_ENABLED`
- web_debug_recorder 新增 `browser_anti_bot_hint` 事件,便于线上回放

**测试**:`tests/test_anti_bot_profile_unit.py`(15 条) + `tests/test_browser_toolkit_throttle_unit.py`(新增 10 条,覆盖 hint 应用 / delay 触发 / 429+503 record_block / success record_request)
...
#### B3.7 落地情况(2026-04-17)

**已落地**:
- `utils/page_fingerprint.py`:`compute_page_hash(url, dom_summary)` 与 `normalize_url_path(url)`
  - URL 归一化:数字段→`:id`,UUID→`:uuid`,16+ 位 hex→`:hash`,24+ 位 base64→`:token`,query/fragment 全部丢弃
  - DOM 结构签名:`page_type` + `page_stage` + 元素/卡片/集合数量 *桶化* + landmark 角色计数(header/nav/main/footer/aside/form/...)
  - 关键属性:文本内容不纳入签名,小幅 DOM 抖动不影响指纹
- `utils/vision_cache.py`:SQLite (`vision_cache(page_hash PK, url_template, description, hit_count, created_at, last_used_at)`)
  - `get(page_hash)` 命中即 `hit_count += 1`,过 TTL 视为 miss;`set()` 写入末尾顺手清理过期行
  - `should_bypass_for_task(task)` 关键字匹配(默认 `login,signin,sign-in,password,payment,checkout,verify,auth,2fa,otp`)
  - 进程级单例 `get_vision_cache()`,`BROWSER_VISION_CACHE_ENABLED=false` 时直接返回 None
- `agents/browser_perception.py`:
  - 新增 `current_task: str` 字段,`BrowserAgent.run()` 入口设置
  - `observe()` 决定要做视觉(`is_new_page` / `complexity` / `unknown_type`)时,先做 `should_bypass_for_task` → 不 bypass 则 `compute_page_hash(url, snapshot)` 查 `VisionCache`,命中直接复用 `description`(同步更新 `_last_vision_url`),miss 走原 `get_vision_description` 路径并 `cache.set(...)`
- `config/settings.py`:`BROWSER_VISION_CACHE_ENABLED` / `BROWSER_VISION_CACHE_DB` / `BROWSER_VISION_CACHE_TTL_DAYS` / `BROWSER_VISION_CACHE_BYPASS_KEYWORDS`
- `CLAUDE.md` "Notable env vars" 与"架构演进记录"同步更新

**测试**:
- `tests/test_page_fingerprint_unit.py`(20 用例):URL 归一化 + page hash 同模板异 ID 折叠 / 异结构发散 / 文本不影响 / 小抖动稳定 / 大跳变发散
- `tests/test_vision_cache_unit.py`(17 用例):get/set / TTL 过期 / 写入触发清理 / bypass 关键字大小写 / disabled gate / stats 计数
- 全部通过(37/37);邻近回归 `tests/test_site_knowledge_store_unit.py` / `tests/test_anti_bot_profile_unit.py` / `tests/test_browser_site_hints_unit.py`(40 条)全绿
- 默认开关 `BROWSER_VISION_CACHE_ENABLED=false`,需要小灰度验证后再切换
...
#### B4.7 落地情况(2026-04-17,与 B2 BrowserToolkit 接入一并)

**已落地**:
- `utils/browser_toolkit.py`:
  - `list_frames(include_main=True)` 返回 `[{index, url, name, is_main, is_detached, domain}]`(比 `list_iframes` 多主 frame + 索引 + host)
  - `list_tabs()` 返回 `[{index, url, title, is_active, is_closed}]`,`title()` 读取外加 0.8s 超时,防跨域阻塞
  - `_enforce_tab_cap()` 在 `goto` / `new_tab` 末尾检查 `BROWSER_MAX_TAB_COUNT`,超限关最老的非 active tab(每次只关一个,下一次再检查)
- `agents/browser_perception.py::observe()`:挂载 `snapshot["available_frames"]` / `snapshot["available_tabs"]`,分别 gate 在 `BROWSER_IFRAME_ENABLED` / `BROWSER_TAB_MANAGEMENT_ENABLED`,失败不阻塞主 observe
- `agents/browser_decision.py`:新增 `_format_available_frames_for_llm` / `_format_available_tabs_for_llm`;`browser_act.txt` / `browser_action_decision.txt` 两条 format 调用都注入新变量,prompt 扩写 `switch_iframe/exit_iframe/switch_tab/close_tab` 为合法动作类型并追加两条决策规则
- `agents/browser_execution.py::try_click/input_with_fallbacks`:主 frame 所有策略都失败后,若 `BROWSER_IFRAME_AUTO_SCAN_ON_STUCK=true`,调 `_iframe_auto_scan_click`/`_iframe_auto_scan_input` 依次 switch_to_iframe 各子 frame 重试同一 selector,命中即留在 iframe,失败回主 frame(名字/URL 两种 iframe 选择器兜底)
- prompt 侧 `browser_unified_plan.txt` 暂未同步更新(legacy 路径,`BROWSER_UNIFIED_ACT_ENABLED=false` 时也会被 `browser_action_decision.txt` 覆盖,数据信号不丢),下次清理再做

**默认开关**:
- `BROWSER_IFRAME_ENABLED=true` / `BROWSER_TAB_MANAGEMENT_ENABLED=true` — 感知暴露默认开,prompt 里多几行成本可忽略
- `BROWSER_IFRAME_AUTO_SCAN_ON_STUCK=false` — 自动扫 iframe 默认关,优先让 LLM 看 `{available_frames}` 主动 SWITCH_IFRAME,稳定后再打开
- `BROWSER_MAX_TAB_COUNT=10` — 超过才回收

**测试**:`tests/test_browser_iframe_tab_unit.py`(新增 10 条,覆盖 list_frames/list_tabs 结构、tab cap 超限触发关老 tab、auto_scan 主 frame 失败后遍历子 frame)
...
#### B5.7 落地情况(2026-04-17,与 B1 执行层注入合并)

**已落地**:
- 新增 `utils/strategy_stats.py`:
  - SQLite 单表 `strategy_stats(domain, role, strategy, success_count, fail_count, total_latency_ms, last_used_at, created_at)` 主键 `(domain, role, strategy)`
  - `StrategyStatsStore`:线程安全、懒加载、feature-flag 完全 no-op;进程单例 `get_strategy_stats_store()`
  - API:`record(domain, role, strategy, *, success, latency_ms)` / `get_stats(domain, role)` / `ranked_strategies(domain, role)` / `skip_strategies(domain, role)`
  - `ranked_strategies`:总样本数 < `BROWSER_STRATEGY_MIN_SAMPLES` 的不参与;成功率 < `BROWSER_STRATEGY_SKIP_THRESHOLD` 的既不 rank 也进 skip 集合;按 (success_rate, success_count) 降序
  - 复用 `utils/site_knowledge_store.normalize_domain` 做 domain 归一化,跟 B1 保持一致
- `agents/browser_execution.py`:
  - fallback chain 尾部统一改成埋点版 loop,每次 try 记录 `(canonical_strategy_name, success, latency_ms)`
  - `_reorder_strategies`:site_hint 永远前置(site-specific 信号压过通用排序);skip 集合里的 strategy 直接从 chain 中剔除;ranked 里的按排序提前,未观测的 strategy 保持原顺序
  - canonical 化规则:`name.split(":", 1)[0]`(`role:button:Submit` → `role`,`site_hint:#x` → `site_hint`,`direct_click` 不变),保证 bucket 稳定
  - 所有写入都在 `BROWSER_STRATEGY_LEARNING_ENABLED=false` 时完全 no-op,零开销
- `config/settings.py` 新增:`BROWSER_STRATEGY_LEARNING_ENABLED` / `BROWSER_STRATEGY_DB` / `BROWSER_STRATEGY_MIN_SAMPLES` / `BROWSER_STRATEGY_SKIP_THRESHOLD`

**测试**:
- `tests/test_strategy_stats_unit.py`(16 条):record / ranked / skip / feature flag / domain/role 隔离
- `tests/test_browser_execution_fallback_unit.py`(10 条):feature off 等价 legacy / site_hint 前置 / site_hint 失败回退 + 双 store 回写 / strategy_stats 成功失败 bucket / skip 过滤 / input 路径同套行为
- 全部通过(16+10/26);邻近回归 `tests/test_site_knowledge_store_unit.py` / `tests/test_browser_site_hints_unit.py` / `tests/test_browser_step_dedup_unit.py` / `tests/test_anti_bot_profile_unit.py` / `tests/test_vision_cache_unit.py` / `tests/test_page_fingerprint_unit.py`(111 条)全绿
- 默认开关 `BROWSER_STRATEGY_LEARNING_ENABLED=false`,需要小灰度验证后再切换
